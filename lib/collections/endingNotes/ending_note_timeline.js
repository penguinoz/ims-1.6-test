...
Meteor.methods({
  ...
  enTimelineGetList: function(userId, isPageOwner, timelineItem, userArray, loginUser, standardDate) {
    if(Meteor.isServer){
    var authCondition = [];
    var authCondition2 = [];
    ...
    if(isPageOwner){
      ...
      bucketCondition = [{lock: true}, {lock: false}];
    } else {
      // 남의 페이지
    ...
    // 타임캡슐 리뉴얼 이후 추가
    var postId = timelineItem.map(function(item) {
      return item.postId;
    });
    ...
    var inArray = [];
    timelineItem.map(function(item, i) {
  ...
  // 타임라인 up버튼
  timeline_upButton: function(userId, userArray, standardDate, type, limit) {
    var aggregate = timelineMain_aggreGt(userId, userArray, standardDate, type, limit);
    // _id 값 배열로 그룹핑
    var group_id = _.map(_.groupBy(aggregate, function(doc) {
      return doc._id;
    }), function(grouped) {
      return _.pluck(grouped, '_id').join(',');
    });
    return CLT.EnTimeline.find({_id: {$in: group_id}}).fetch();
  },
  // 타임라인 up버튼 count
  ...
  // 타임라인 down버튼
  timeline_downButton: function(userId, userArray, standardDate, type, limit) {
    var aggregate = timelineMain_aggreLte(userId, userArray, standardDate, type, limit);
    // _id 값 배열로 그룹핑
    var group_id = _.map(_.groupBy(aggregate, function(doc) {
      return doc._id;
    }), function(grouped) {
...
var timelineMain_aggreLte = function(userId, userArray, standardDate, type, limit) {
  var friendType = type;
  if (type === 'ALL') {
    friendType = {$in: ['BL','IM','BS']};
    type = /^/;
  }
  var frineds = _.without(userArray, userId);
  if (Meteor.isServer) {
    	...
    	[
    		...
    		// Stage 2
    		{
    			$project: {
    			  ...
    			  timelineDate:1,
    			  sort:1,
    			  targetUser: {$cond: { if: {$eq: [ "$userId", userId ]}, then: '', else: userId }}
    			}
    		},
    		...
    		// Stage 7
    		{
    			$project: {
    			  ...
    			  type:1,
    			  sort:1,
    			  useYn: { $eq: [ "$friends.userId", "$userId" ] },
    			  timelineDate : {$cond: { if: {$eq: [ "$userId", userId ]}, then: '$timelineDate', else: {$cond: { if: {$lte: [ "$friends.regDate", '$regDate' ]}, then: {$substr:['$regDate',0,10]}, else: '' }}}},
    			}
    		},
    		...
    		// Stage 8
    		{
    			$match: {
    				...
    				timelineDate :{$lte: standardDate, $ne:''}
    			}
    		},
...
var timelineMain_aggreGt = function(userId, userArray, standardDate, type, limit) {
  var friendType = type;
  if (type === 'ALL') {
    friendType = {$in: ['BL','IM','BS']};
    type = /^/;
    	...
    	[
    		...
    		// Stage 8
    		{
    			$match: {
    				...
    				timelineDate :{$gt: standardDate, $ne:''}
    			}
    		},
