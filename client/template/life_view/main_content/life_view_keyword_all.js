...
// 라이프뷰 키워드로 보기
var templateName = 'lifeViewKeywordAll';
...
Template[templateName].onRendered(function(){
  ...
  Meteor.call('getTagListAll', global.login.userId, function(error, result) {
    ...
    if (error) {
      ...
      linkTag = linkTag.map(function(d){
        if (d[3] < linkWdL[0][0]){ d[3] = d[3] * linkWdL[0][1];
        } else if (d[3] < linkWdL[1][0]){ d[3] = d[3]*linkWdL[1][1];
        } else d[3] = d[3]*linkWdL[2][1];
