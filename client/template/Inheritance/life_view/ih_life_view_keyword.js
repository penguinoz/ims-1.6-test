...
var templateName = 'ihLifeViewKeyword';
...
function keyword(data) {
  ...
  var instance = data;
  Meteor.call('getTagListByPostId', data.postId, function(error, result) {
    if (error) {
      return alert(error);
    } else {
      var addTest = [];
      ...
      nodeTag = nodeTag.map(function(d){
        return {"id": d[0], "wrts": d[1], "size":d[2]*unitCirSize, "score": d[2]/maxCount, "type": "circle"};
      });
      ...
      linkTag = linkTag.map(function(d){
        // console.log('linkTag', d);
        if (d[3] < linkWdL[0][0]) {
          d[3] = d[3] * linkWdL[0][1];
          // d[3] = linkWdL[0][1];
        } else if (d[3] < linkWdL[1][0]) {
          d[3] = d[3]*linkWdL[1][1];
          // d[3] = linkWdL[0][0];
        } else {
          d[3] = d[3]*linkWdL[2][1];
        ...
        return {"source": d[0], "target": d[1], "wrts":d[2], "size": d[3]};
      });
      ...
      function node_detail_highlight(d) {
        ...
        Meteor.call('getStoryBucketData', postId, global.login.userId, function(error, result) {
          if (error) {
            ...
            if (result.length === 1) {
              ...
              global.fn_replaceLifeViewDetail(selectObj, 'ihLifeView');
            } else {
              var templateData = {};
              templateData.contentTmp = 'ihLifeViewDetailList';
