...
// 마이페이지 > 댓글
var templateName = 'customerWritePopup';
var instance;
var imageListTemp = [];
var removeImageList=[];
...
Template[templateName].onRendered(function(){
  ...
  if(this.data){
    $("#customerTitle").val(this.data.title);
    this.$('div.froala-reactive-meteorized').froalaEditor('html.set',this.data.question);
  }
});

Template[templateName].helpers({
  hpFileListSet: function(_file){
    var files = [];
    if(_file){
      if(instance.fileList.get()){
        files = instance.fileList.get();
        files.push(_file);
      } else {
        files.push(_file);
      }
      instance.fileList.set(files);
    }
  },
  hpFileList: function(){
...
Template[templateName].events({
  "click #saveWriteQna": function(e, t){
    e.preventDefault();
    var titleParam = $("#customerTitle").val();
    var quesParam = t.$('div.froala-reactive-meteorized').froalaEditor('html.get', true, {imagePasteProcess: true});
    if(!titleParam.trim() || !quesParam.trim()){
      global.utilAlert("제목과 내용을 작성해 주세요.");
      return;
    }
    ...
    if (global.utilValidation(t)) {
      setTimeout(function(){
        ...
        //   insertObj.title = $("#customerTitle").val();
        ...
        Meteor.call('insertQna', uniqId ,saveWriteQnaData, function(error, result) {
          if (error) {
            ...
            setTimeout(function(){
              ...
              if(result.insertedId){
                templateData.data.selectedId = result.insertedId;
              }else{
                templateData.data.selectedId = instance.data._id;
              }
              Session.set('customerMain templateData', templateData);
            }, 200);
          }
  ...
  //   var insertObj = {};
  //   var triger = 0;
  //   if(!$("#question").val().trim() || !$("#customerTitle").val().trim()){
  //     global.utilAlert("제목과 내용을 모두 작성해 주세요.");
  //     return;
  //   }
  //   insertObj.title = $("#customerTitle").val();
  //   insertObj.question = $("#question").val();
  ...
  //   insertObj.ansfile = [];
  //   var files = [];
  //   _.each(instance.fileList.get(), function(data){
  //     if(data){
  //       var nameString = data.path;
  //       var newFileName =  Meteor.uuid()+ "custImg-Path" + data.name;
  //       // var pathId = "customer_desk/"+newFileName+ "." + nameString.substring(nameString.lastIndexOf('.')+1);
  //       var pathId = "customer_desk/"+newFileName
  //       var pushObj = {
  //         file:data.file,
  //         name : data.file.name,
  ...
  //   var modalobj = {};
	...
	// 	modalobj.size = 'imsr-pop modal-sm';
	// 	modalobj.fade = true;
  ...
  "click #frListTag":function(e, t){
    var fileArr = t.fileList.get();
    for(var i in fileArr){
      if(fileArr[i].path === this.path){
        fileArr.splice(i,1);
        t.fileList.set(fileArr);
        break;
      }
    }
  }
