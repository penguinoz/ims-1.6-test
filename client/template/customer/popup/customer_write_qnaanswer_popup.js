...
// 마이페이지 > 댓글
var templateName = 'customerWriteQnaanswerPopup';
var instance;
var imageListTemp;
var removeImageList;
...
Template[templateName].onRendered(function(){
  ...
  if(this.data && this.data.answer){
    this.$('div.froala-reactive-meteorized').froalaEditor('html.set',this.data.answer);
  }
});
...
Template[templateName].events({
  "click #saveWriteQna": function(e, t){
    e.preventDefault();
    var answParam = t.$('div.froala-reactive-meteorized').froalaEditor('html.get', true, {imagePasteProcess: true});
    if(!answParam.trim()){
    ...
    if (global.utilValidation(t)) {
      setTimeout(function(){
        ...
        Meteor.call('upsertAnswer', uniqId ,answParam, image, function(error, result) {
          if (error) {
